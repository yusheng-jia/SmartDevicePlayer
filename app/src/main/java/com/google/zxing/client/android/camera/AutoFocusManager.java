...
final class AutoFocusManager implements Camera.AutoFocusCallback {
  ...
  AutoFocusManager(Camera camera) {
    ...
    Glog.i(TAG, "Current focus mode '" + currentFocusMode + "'; use auto focus? " + useAutoFocus);
    start();
  }
  ...
  private synchronized void autoFocusAgainLater() {
    if (!stopped && outstandingTask == null) {
      ...
      try {
        ...
        Glog.w(TAG, "Could not request auto focus", ree);
      }
    }
  }
    ...
    if (useAutoFocus) {
      ...
      if (!stopped && !focusing) {
        try {
          ...
          Glog.w(TAG, "Unexpected exception while focusing", re);
  ...
  synchronized void stop() {
    ...
    if (useAutoFocus) {
      ...
      try {
        ...
        // Have heard RuntimeException reported in Android 4.0.x+; continue?
        Glog.w(TAG, "Unexpected exception while cancelling focusing", re);
      }
    }
  }

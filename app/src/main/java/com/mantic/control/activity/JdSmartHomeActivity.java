 ...
 * desc:
 */
public class JdSmartHomeActivity extends SwipeBackActivity implements TitleBar.OnButtonClickListener {
    private static final String TAG = JdSmartHomeActivity.class.getSimpleName();
    ...
    Button deviceButton;
    @BindView(R.id.jd_skill_titlebar)
    TitleBar titleBar;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_jd_skill);
        mContext = this;
        ButterKnife.bind(this);
        accountServiceApi =  AccountRetrofit.getInstance().create(AccountServiceApi.class);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            StatusBarUtil.setColor(JdSmartHomeActivity.this, Color.parseColor("#f9f9fa"), 0);
    ...
    public void deviceClick(){
        Intent intent = new Intent(this,JdVirtualDevicesActivity.class);
    ...
    private void openDevice(){
        boolean status = SharePreferenceUtil.getSmartHomeOpen(this);
        if (status){
            setJdSwitch(false);
        }else {
            setJdSwitch(true);
    ...
    private void updateStatus(){
        Glog.i(TAG,"updateStatus ... ");
        if (isAuthorize()){
            if (SharePreferenceUtil.getSmartHomeOpen(mContext)){
    ...
    private void setJdSwitch(Boolean value){
        ...
        Boolean voiceValue = SharePreferenceUtil.getVoiceShoppingOpen(this);
        String request = "{\"user_id\":\""+ user_id+"\",\"device_uuid\":\""+ deviceId+"\",\"jd_skill_smart_home_switch\":"+value+",\"jd_skill_shopping_switch\":"+voiceValue+"}";
        Glog.i(TAG,"request: " + request);
        RequestBody body = RequestBody.create(okhttp3.MediaType.parse("application/json; charset=utf-8"),request);
        Map<String,String> map = new HashMap<>();
        map.put("Content-Type","application/json");
                ...
                if (response!=null && response.body() != null){
                    try {
                        ...
                        if (mainObject.getString("retcode").equals("0")){//设置成功
                            ...
                            SharePreferenceUtil.setSmartHomeOpen(mContext,smartSwitch);
                            if (smartSwitch){
